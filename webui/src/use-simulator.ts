...
export type RareEvent = { id: number; type: "fault" | "reset" | "diagnostic" | "scenario_step"; timestamp: string; payload: Record<string, unknown> };
...
export function useSimulator() {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      const consume = (event: MessageEvent<string>) => {
        const message = JSON.parse(event.data) as { id: number; type: string; revision: number | null; timestamp: string; payload: { points?: PointValue[]; changes?: PointValue[]; from_revision?: number; [key: string]: unknown } };
        ...
        if (["fault", "reset", "diagnostic", "scenario_step"].includes(message.type)) {
          setLastEvent({ id: message.id, type: message.type as RareEvent["type"], timestamp: message.timestamp, payload: message.payload });
        }
      };
      ...
      source.onerror = () => { if (!closed) setStream("reconnecting"); };
    };
  ...
  return useMemo(() => ({ status, catalog, points, modelTime, stream, lastEvent }), [status, catalog, points, modelTime, stream, lastEvent]);
}
